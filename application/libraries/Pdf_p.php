...
#----------------------------------------------------------------------------------------------------------------------------------
	...
	function Header(){
      ...
      //FUNCION PARA TRUNCAR HASTA - EN EL NOMBRE DE EMPRESA
	  $posicionGuion=strpos($this->empresa->nom_emp,'-');
	  $empresa_name='';
	  if($posicionGuion===false){
		$empresa_name=$this->empresa->nom_emp;
	  }else{
		$empresa_name=substr($this->empresa->nom_emp,0 , $posicionGuion);
	  }
	  $this->MultiCell(110,4,utf8_decode($empresa_name).'  RUC: '.utf8_decode($this->empresa->ruc_emp) ,0,'C');
      $this->SetXY(50,13);
