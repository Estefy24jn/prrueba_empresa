...
class RetencionSRI extends ComprobanteSRI
{
  ...
  function CreaDivParaPDF($objXml="", $pdf){
    $registro = $this->objmodel->datosRetencionCompra($this->id);
    $telefonoproveedor =  $registro->telf_proveedor;
    $emailproveedor =  $registro->correo_cliente;
    $direccionproveedor =  $registro->direccion_proveedor;
      ...
      $divstr.='<tr>';
        ...
        $divstr.='<div class="cuadro_izq" style="float:left; width:350px; ">';
            ...
            $divstr.='<div style="border:1px solid #000; padding:5px; border-radius:5px; margin-right:5px; font-size:12px  ">';
              ...
              $divstr.='<div class="row"  style="font-size:10px ;margin:10px 0px;"> Dirección Sucursal: '.$xmlFact->infoCompRetencion->dirEstablecimiento.'</div>';
              $divstr.='<div class="row"  style="font-size:11px;margin: 12px 0px; "> OBLIGADO A LLEVAR CONTABILIDAD: '.$xmlFact->infoCompRetencion->obligadoContabilidad.' </div>';
            $divstr.='</div>';
        $divstr.='</div>';
        ...
        $divstr.='<table style="padding: 1.3px;">';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:16px;"> R.U.C: '.$xmlFact->infoTributaria->ruc.' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; COMPROBANTE DE RETENCIÓN </td>';
          $divstr.='</tr>';
          /* $divstr.='<tr>';
          ...
          $divstr.='<tr>';
            $facNumber = substr($xmlFact->infoTributaria->claveAcceso,24,15);
            $facNumber = substr($facNumber,0,3).'-'.substr($facNumber,3,3).'-'.substr($facNumber,6,9);   
          $divstr.='<td style="font-size:14px;"> No. &nbsp;&nbsp;&nbsp;'.$facNumber.'</td>';
          $divstr.='</tr>';
 /*          $divstr.='<tr>';
          $divstr.='<td style="font-size:6px;"> </td>';
          $divstr.='</tr>'; */
          $divstr.='<tr>';
          $divstr.='<td style="font-size:12px;"> NÚMERO DE AUTORIZACIÓN: </td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          ...
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:12px;"> FECHA Y HORA DE AUTORIZACIÓN: </td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:12px;"> '.$objXml->fechaAutorizacion.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
            $ambiente = ($xmlFact->infoTributaria->ambiente == 1) ? "PRUEBAS":"PRODUCCIÓN";
          $divstr.='<td style="font-size:12px;"> AMBIENTE: '.$ambiente.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
            $tipoemision = ($xmlFact->infoTributaria->tipoEmision == 1) ? "NORMAL":"INDISPONIBILIDAD DEL SISTEMA";
          $divstr.='<td style="font-size:12px;"> EMISIÓN: '.$tipoemision.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:12px;"> CLAVE DE ACCESO: </td>';
          $divstr.='</tr>';
          $divstr.='<tr style="height: 40px;">';
            $facNumber = substr($xmlFact->infoTributaria->claveAcceso,0,49);
      ...
      $divstr.='<div clas="datos_cliente" style="clear:left;border:1px solid #000; margin:12px 0px">';
        $divstr.='<table>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:14px; width: 70%;">Razón Social / Nombres: '.$xmlFact->infoCompRetencion->razonSocialSujetoRetenido.'</td>';
          $divstr.='<td style="font-size:14px; width: 30%;">Fecha Emisión: '.$xmlFact->infoCompRetencion->fechaEmision.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:14px;">Identificación: '.$xmlFact->infoCompRetencion->identificacionSujetoRetenido.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:14px;">Teléfono: '.$telefonoproveedor.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:14px;">Email: '.$emailproveedor.'</td>';
          $divstr.='</tr>';
          $divstr.='<tr>';
          $divstr.='<td style="font-size:14px;">Dirección: '.$direccionproveedor .'</td>';
          $divstr.='</tr>';
        $divstr.='</table>';
      $divstr.='</div>';          
