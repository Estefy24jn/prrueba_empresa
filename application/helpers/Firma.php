...
class Firma
{
        ...
        try {
            ...
            if (openssl_pkcs12_read(file_get_contents($pathFirma, FILE_USE_INCLUDE_PATH), $certs, $this->config['pass'])) {
                ...
                if (array_key_exists('O', $certf['subject'])){
                    ...
                    if ($certificante === "BANCO CENTRAL DEL ECUADOR") {
                        ...
                        foreach ($aux as $item) {
                            ...
                            $this->certificate = $x509cert;
                            $this->certData = $certf;
                    } else if ($certificante === "ARMIJOS RIVERA ALVARO DAVID") {
                            $this->certificate = $x509cert;
                            $this->certData = $certf;
                    } else {
                        $this->certificate = $x509cert;
                ...
                if ($this->certificate === null || $this->certificate === false)
                    return array('error' => true, 'mensaje' => "No existe certificado valido.", 'log' => $logFirma);
    ...
    {
        //return json_encode($this->certData); 
        return date('Y-m-d', $this->certData['validTo_time_t']);
    }
