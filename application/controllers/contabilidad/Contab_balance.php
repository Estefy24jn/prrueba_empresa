...
class Contab_balance extends CI_Controller {
    ...
    public function listadoCashFlow() {
        $hasta = $this->session->userdata("tmp_balsit_hasta");
        $nivel = $this->session->userdata("tmp_balsit_nivel");
        $sucursal = $this->session->userdata("tmp_balsit_sucursal");
        $pendiente = $this->session->userdata("tmp_balsit_pendiente");
        $registro = $this->Contab_balance_model->sel_perdidasyganancias($sucursal, $hasta, $nivel, $pendiente);
        $tablacashflow = "";
        $pyg = 0.0;
        foreach ($registro as $row) {
            /* $tabla.='{  "id":"' .$row->id. '",
        ...
        /* Tabla de Cashflow */
        /* Con Abs pongo el absoluto para el reporte */
        $tablacashflow.='{"cuenta":"Ingresos-Costos-Gastos ",
            "valor":"' .abs($pyg). '"
        }';
        //$this->session->set_flashdata('pyg', $pyg);

        /* Enviar tabla */
        /* $tabla = substr($tabla, 0, strlen($tabla) - 1); */
        //echo '{"data":[' . $tabla . '],"cashflow":[' . $tablacashflow . ']}';
        echo '{"data":[' . $tablacashflow . ']}';//Original
        
    }

    public function listadoPerdidasyGanancias() {
        $hasta = $this->session->userdata("tmp_balsit_hasta");
        $nivel = $this->session->userdata("tmp_balsit_nivel");
        $sucursal = $this->session->userdata("tmp_balsit_sucursal");
        $pendiente = $this->session->userdata("tmp_balsit_pendiente");
