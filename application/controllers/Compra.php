...
class Compra extends CI_Controller {
    ...
    public function index() {
        ...
        if ($desde == ''){
            $desde = date("Y-m-d"); 
            $desde = date('Y-m-d', strtotime( "$desde - 5 day" )); 
            $hasta = date("Y-m-d");
...
/*     public function construirexcell($arrayxml){
        $inputFileName = FCPATH.'public/upload/SRIDescargados.xlsx';
        try{
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A2','515151');
        //Guardar el archivo excel con este nombre;
        $filename='Plantilla_ATS.xlsx'; //save our workbook as this file name
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); //mime type
        header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
        header('Cache-Control: max-age=0'); //no cache
        //se necesita un objeto Writer para guardar el excel 
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');  
        $objWriter->save('php://output');     
        //$objWriter->save('Nimit New.xlsx');
    } */
    public function descargarxml($resclaveAcceso){
        //echo($res);
        $url = "https://cel.sri.gob.ec/comprobantes-electronicos-ws/AutorizacionComprobantesOffline?wsdl";
        $params = array("claveAccesoComprobante" => $resclaveAcceso);
        $client = new soapClient($url, array("connection_timeout" => 35000));
        $result = $client->autorizacionComprobante($params);
        if ($result){
            if($result->RespuestaAutorizacionComprobante->numeroComprobantes <> 0){
                //$result->RespuestaAutorizacionComprobante->autorizaciones->autorizacion->estado
                /* //Para ver como es el array 
                $data = $result->RespuestaAutorizacionComprobante->autorizaciones->autorizacion;
                $data= json_decode( json_encode($data), true);
                //print_r ($data); */
                //Obtengo XML de comprobante
                //print ($result->RespuestaAutorizacionComprobante->autorizaciones->autorizacion->comprobante);
                return $result->RespuestaAutorizacionComprobante->autorizaciones->autorizacion->comprobante;
                
                //Construir excel con los datos y poner en la ultima fila de excel
                //Tengo xml de documento
                //Guardar un array de xml
            }
        }else{
            print $result;
        }
        echo "Hola esto es eso \n";
    }

    public function descargar_sri(){
        $name = FCPATH.'public/upload/tempsri.xlsx';
    ...
    public function cargar_sritxt() {
        ...
        if ( 0 < $_FILES['file']['error'] ) {
            ...
            foreach($array as $fila ){
                //Si es un documento factura, ND, NC, retencion
                //if (strpos($fila, 'Factura') !== false  || strpos($fila, 'Notas de Débito') !== false || strpos($fila, 'Notas de Crédito') !== false || strpos($fila, 'Comprobante de Retención') !== false) {
                if (strlen($fila)>=49 ) {
                    // Texto contains sri num
                    //Obtener el final de la cadena
                    $res = substr($fila, (strlen($fila)-49),strlen($fila));
                    if($res!=false && is_numeric($res)){
                        $xml = $this->descargarxml($res);
                        $this->excel->getActiveSheet()->setCellValue('R'.$i, $res );
                        $this->excel->getActiveSheet()->setCellValue('S'.$i, $xml );
                        array_push($sricomprobantes, $xml);
            ...
            //$this->construirexcell($sricomprobantes);
            //$res = $array[0];
        }
        //PASOS
        //1. Cargar txt
        //2. Obtener num autorizaciones de compra, NC, ND, Retenciones
        //3. Con cada numero tengo que autorizar factura a wsdl de sri
        //4. Obtengo los datos de todos los comprobantes en xml
        //5. Visualizar datos (Para contadores) - Construir excel de comprobantes
        //6. Importar - Guardo en compras, NC, ND, 
        
        //$name = '/path/to/folder/xyz.xlsx';
        //Guardar Excel temporal en server
        $this->excel->setActiveSheetIndex(0);
        //$objPHPExcel->getActiveSheet()->getStyle('A2')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
        $this->excel->getActiveSheet()->setCellValue('A1','RUC');
        $this->excel->getActiveSheet()->setCellValue('B1','FECHA');
        $this->excel->getActiveSheet()->setCellValue('C1','FACTURA');
        $this->excel->getActiveSheet()->setCellValue('D1','TIPO DOC');
        $this->excel->getActiveSheet()->setCellValue('E1','PROVEEDOR');
        $this->excel->getActiveSheet()->setCellValue('F1','RUC PROVEEDOR');
        $this->excel->getActiveSheet()->setCellValue('G1','BASE IMP 12');
        $this->excel->getActiveSheet()->setCellValue('H1','BASE IMP 0');
        $this->excel->getActiveSheet()->setCellValue('I1','BASE EXENTO');
        $this->excel->getActiveSheet()->setCellValue('J1','IVA 12');
        $this->excel->getActiveSheet()->setCellValue('K1','ICE');
        $this->excel->getActiveSheet()->setCellValue('L1','IRBPNR');
        $this->excel->getActiveSheet()->setCellValue('M1','SUBTOTAL');
        $this->excel->getActiveSheet()->setCellValue('N1','TOTAL');
        $this->excel->getActiveSheet()->setCellValue('O1','DOC MODIFICADO');
        $this->excel->getActiveSheet()->setCellValue('P1','AUTORIZACIÓN');
        ...
        /* $this->excel->getActiveSheet()->getStyle('C')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_GENERAL);
        $this->excel->getActiveSheet()->getStyle('D')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_GENERAL);
        $this->excel->getActiveSheet()->getStyle('E')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_GENERAL); */
//GUARDAR XML
        $fila = 2;
        //$contador = 0;
        foreach($sricomprobantes as $registrotemp){
            //$this->excel->getActiveSheet()->setCellValue('D'.$fila, $registrotemp );
            $xml = simplexml_load_string($registrotemp);
            //print $xml->infoFactura->totalSinImpuestos;
            //$this->excel->getActiveSheet()->setCellValue('C'.$fila, $xml->infoTributaria->estab . '-' . $xml->infoTributaria->ptoEmi . '-' . $xml->infoTributaria->secuencial );
            
            //$this->excel->getActiveSheet()->setCellValue('E'.$fila, $contador );
            //$this->excel->getActiveSheet()->setCellValue('F'.$fila, count($sricomprobantes) );
            //$contador++;
            $this->excel->getActiveSheet()->setCellValue('C'.$fila, $xml->infoTributaria->estab . '-' . $xml->infoTributaria->ptoEmi . '-' . $xml->infoTributaria->secuencial );
            ...
            $tipodoc="";
            $infoparadoc=array();
            switch($xml->infoTributaria->codDoc){
                case 01:
                    $tipodoc="Factura";
                    $infoparadoc= $xml->infoFactura->totalConImpuestos->totalImpuesto;
                    $this->excel->getActiveSheet()->setCellValue('A'.$fila, $xml->infoFactura->identificacionComprador);
                    $this->excel->getActiveSheet()->setCellValue('B'.$fila, $xml->infoFactura->fechaEmision);
                    $this->excel->getActiveSheet()->setCellValue('M'.$fila, $xml->infoFactura->totalSinImpuestos);
                    $this->excel->getActiveSheet()->setCellValue('N'.$fila, $xml->infoFactura->importeTotal);
                    break;
                case 04:
                    $tipodoc="Nota de Crédito";
                    $infoparadoc= $xml->infoNotaCredito->totalConImpuestos->totalImpuesto;
                    $this->excel->getActiveSheet()->setCellValue('A'.$fila, $xml->infoNotaCredito->identificacionComprador);
                    $this->excel->getActiveSheet()->setCellValue('B'.$fila, $xml->infoNotaCredito->fechaEmision);
                    $this->excel->getActiveSheet()->setCellValue('M'.$fila, $xml->infoNotaCredito->totalSinImpuestos);
                    $this->excel->getActiveSheet()->setCellValue('N'.$fila, $xml->infoNotaCredito->valorModificacion);
                    $this->excel->getActiveSheet()->setCellValue('O'.$fila, $xml->infoNotaCredito->numDocModificado);
                    break;
                case 05:
                    $tipodoc="Nota de Débito";
                    $infoparadoc= $xml->infoNotaDebito->impuestos->impuesto;
                    $this->excel->getActiveSheet()->setCellValue('A'.$fila, $xml->infoNotaDebito->identificacionComprador);
                    $this->excel->getActiveSheet()->setCellValue('B'.$fila, $xml->infoNotaDebito->fechaEmision);
                    $this->excel->getActiveSheet()->setCellValue('M'.$fila, $xml->infoNotaDebito->totalSinImpuestos);
                    $this->excel->getActiveSheet()->setCellValue('N'.$fila, $xml->infoNotaDebito->valorTotal);
                    $this->excel->getActiveSheet()->setCellValue('O'.$fila, $xml->infoNotaDebito->numDocModificado);
                    break;
                case 07:
                    $tipodoc="Retención";
                    $infoparadoc= $xml->impuestos->impuesto;
                    $this->excel->getActiveSheet()->setCellValue('A'.$fila, $xml->infoCompRetencion->identificacionSujetoRetenido);
                    $this->excel->getActiveSheet()->setCellValue('B'.$fila, $xml->infoCompRetencion->fechaEmision);
                    break;
                default:
                    $tipodoc=$xml->infoTributaria->codDoc;
            }
            //NOMBRE DE EMPRESA O DE PERSONA NATURAL
            $nombreemp="";
            if($xml->infoTributaria->nombreComercial == "" || $xml->infoTributaria->nombreComercial == " "){
                $nombreemp = $xml->infoTributaria->razonSocial;
            }else{
                $nombreemp = $xml->infoTributaria->nombreComercial;
            }
            $this->excel->getActiveSheet()->setCellValue('D'.$fila, $tipodoc );
            $this->excel->getActiveSheet()->setCellValue('E'.$fila, $nombreemp );
            $this->excel->getActiveSheet()->setCellValue('F'.$fila, $xml->infoTributaria->ruc );
            ...
            foreach ($infoparadoc as $impuesto) {
                if($impuesto->codigo=="2" && $impuesto->codigoPorcentaje=="2"){ //Si tiene IVA
                    
                    //IVA 12
                    $this->excel->getActiveSheet()->setCellValue('J'.$fila, $impuesto->valor );
                    //Poner base imponible 12
                    $this->excel->getActiveSheet()->setCellValue('G'.$fila, $impuesto->baseImponible );
                }else if($impuesto->codigo=="2" && $impuesto->codigoPorcentaje=="0"){
                    //Poner base imponible 0
                    //BASE IMP 0
                    $this->excel->getActiveSheet()->setCellValue('H'.$fila, $impuesto->baseImponible );
                }
                else if($impuesto->codigo=="2" && ($impuesto->codigoPorcentaje=="6" || $impuesto->codigoPorcentaje=="7" )){
                    //BASE Exento
                    $this->excel->getActiveSheet()->setCellValue('I'.$fila, $impuesto->baseImponible );
                }
                else if($impuesto->codigo=="3"){
                    //ICE
                    $this->excel->getActiveSheet()->setCellValue('K'.$fila, $impuesto->valor );
                }
                else if($impuesto->codigo=="5"){
                    //IRBPNR
                    $this->excel->getActiveSheet()->setCellValue('L'.$fila, $impuesto->valor );
                }
                else if($impuesto->codigo=="2" && $tipodoc=="Retención"){
                    //RETENCION IVA
                     $this->excel->getActiveSheet()->setCellValue('J'.$fila, $impuesto->valorRetenido );
                     //Poner base imponible 12
                     $this->excel->getActiveSheet()->setCellValue('G'.$fila, $impuesto->baseImponible );
                     //Doc Sustento retencion
                     $this->excel->getActiveSheet()->setCellValue('P'.$fila, $impuesto->numDocSustento );
                }
                else if($impuesto->codigo=="1" && $tipodoc=="Retención"){
                    //RETENCION IVA
                     $this->excel->getActiveSheet()->setCellValue('M'.$fila, $impuesto->valorRetenido );
                     //Poner base imponible 12
                     $this->excel->getActiveSheet()->setCellValue('N'.$fila, $impuesto->baseImponible );
                     //Doc Sustento retencion
                     $this->excel->getActiveSheet()->setCellValue('O'.$fila, $impuesto->numDocSustento );
                } 
            }
            
            //print_r($xml);
            $this->excel->getActiveSheet()->setCellValue('P'.$fila, $xml->infoTributaria->claveAcceso);
            $fila++;
        }
...
//FIN
        ...
        print json_encode($sricomprobantes); 
    }

    
    

    public function listadofacturasaimportar(){
        //$tmpfile = $this->session->userdata("tmp_impcta_file");
        $tmpfile = "tempsri.xlsx";
        $tabla = "";
        ...
        if ($tmpfile != ''){
            ...
            if (file_exists($tmpfname)){
                ...
                for ($row = 2; $row <= $lastRow; $row++) {
                    $tabla.='{  "ruc":"' .addslashes($worksheet->getCell('A'.$row)->getValue()). '",
                                "fecha":"' .addslashes($worksheet->getCell('B'.$row)->getValue()). '",
                                "factura":"' .addslashes($worksheet->getCell('C'.$row)->getValue()). '",
                                "tipodoc":"' .addslashes($worksheet->getCell('D'.$row)->getValue()). '",
                                "proveedor":"' .addslashes($worksheet->getCell('E'.$row)->getValue()). '",
                                "rucproveedor":"' .addslashes($worksheet->getCell('F'.$row)->getValue()). '",
                                "base12":"' .addslashes($worksheet->getCell('G'.$row)->getValue()). '",
                                "base0":"' .addslashes($worksheet->getCell('H'.$row)->getValue()). '",
                                "baseex":"' .addslashes($worksheet->getCell('I'.$row)->getValue()). '",
                                "iva12":"' .addslashes($worksheet->getCell('J'.$row)->getValue()). '",
                                "ice":"' .addslashes($worksheet->getCell('K'.$row)->getValue()). '",
                                "irbp":"' .addslashes($worksheet->getCell('L'.$row)->getValue()). '",
                                "subtotal":"' .addslashes($worksheet->getCell('M'.$row)->getValue()). '",
                                "total":"' .addslashes($worksheet->getCell('N'.$row)->getValue()). '",
                                "docmodificado":"' .addslashes($worksheet->getCell('O'.$row)->getValue()). '",
                                "autorizacion":"' .addslashes($worksheet->getCell('P'.$row)->getValue()). '"
                            },';
                } 
            }    
        }
    ...
    /* Chequear si ya se a ingresado una compra*/
    public function check_repetida(){
        $factura = $this->input->post('factura');
        $valor = $this->Compra_model->check_nro_factura_repetida($factura);
        print json_encode($valor);//Envia 1 si esta repetida y 0 si no
    } 
...
/* ====================================================================================== */
    ...
    public function guardar_retencion(){
        ...
        $idcompra = $this->input->post('txt_id_comp');
...
*/
        $this->Retencion_model->retencioncompra_guardar($idcompra, $autorizacion, $fecha, $retiva10, $retiva20, $retiva30, $retiva50, $retiva70, $retiva100, $ptoemision, $nroretencion);
