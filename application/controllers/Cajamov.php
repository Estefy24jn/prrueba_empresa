...
*/
    ...
    public function resumencaja_pdf(){
      ...
      //Las ventas del día
      $ventasmov = $this->facturar_model->venta_detalles_rango($resmov->fecha_apertura, $resmov->fecha_cierre, $resmov->id_usuario, $resmov->id_sucursal); 
    ...
    //  $this->pdf_caja->Cell(182,7,utf8_decode(" $resmov->observaciones"),1,1,'L');
      ...
      /*Resumen de Ventas durante apertura de la caja */
      /*Nueva Pagina para reporte de Ventas */
      $this->pdf_caja->AddPage('L', 'A4');        #Orientación y tamaño P vertival L Horizontal
      $this->pdf_caja->ln(6); 
      $this->pdf_caja->Cell(275,7,utf8_decode("Resumen de Ventas"),1,1,'C');
      $this->pdf_caja->Cell(40,7,utf8_decode(" Fecha"),1,0,'C');
      $this->pdf_caja->Cell(90,7,utf8_decode(" Descripción"),1,0,'C');
      $this->pdf_caja->Cell(15,7,utf8_decode(" Cant"),1,0,'C');
      $this->pdf_caja->Cell(70,7,utf8_decode(" Cliente"),1,0,'C');
      $this->pdf_caja->Cell(40,7,utf8_decode(" Factura"),1,0,'C');
      $this->pdf_caja->Cell(20,7,utf8_decode(" Monto"),1,1,'C');

       foreach ($ventasmov as $row) {
        $fecha = $row->fecharegistro;
        $descripcion = $row->descripcion;
        $numfac = $row->nro_factura;
        $cliente = $row->nom_cliente;
        $cantidad = $row->cantidad;
        $monto = number_format($row->valortotal,2);

        $this->pdf_caja->Cell(40,7,utf8_decode(" $fecha"),1,0,'L');
        $this->pdf_caja->Cell(90,7,utf8_decode(" $descripcion"),1,0,'L');
        $this->pdf_caja->Cell(15,7,utf8_decode(" $cantidad"),1,0,'L');
        $this->pdf_caja->Cell(70,7,utf8_decode(" $cliente"),1,0,'L');
        $this->pdf_caja->Cell(40,7,utf8_decode("$numfac"),1,0,'C');
        $this->pdf_caja->Cell(20,7,utf8_decode(" $monto"),1,1,'R');
