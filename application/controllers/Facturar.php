...
class Facturar extends CI_Controller {
    ...
    /* ABRE LA VENTANA PARA MOSTRAR EL DETALLE DE LA TARJ.CREDITO */
    public function ventaformapagotarjetaporid(){
      $idventa = $this->input->post("id");
      /* Cargar Detalle de Forma de Pago */
      $forpagodetalle = $this->facturar_model->ventaformapagotarjetaporid($idventa);
      //return var_dump($forpagodetalle[0]);
      $data["forpagodetalle"] = $forpagodetalle[0];
      $this->load->view("factura_tipopagodetalle", $data);
    }

    /* ABRE LA VENTANA PARA MOSTRAR EL DETALLE DE LA TARJ.CREDITO */
    public function ventaformapagochequeporid(){
      $idventa = $this->input->post("id");
      /* Cargar Detalle de Forma de Pago */
      $forpagodetalle = $this->facturar_model->ventaformapagochequeporid($idventa);
      //return var_dump($forpagodetalle[0]);
      $data["forpagodetalle"] = $forpagodetalle[0];
      $this->load->view("factura_tipopagodetallecheque", $data);
    }
    ...
    public function ventas(){
      ...
      if (($desde == NULL) || ($hasta == NULL)){
        $fdesde = date("Y-m-d");
        $fdesde = date('Y-m-d', strtotime( "$fdesde - 5 day" )); 
        $fhasta = date("Y-m-d"); 
...
//        $tabla.="Producto     Cant.    P.Unit   P.Tot". "\r\n";        
        ...
        foreach ($registro as $row) {
            ...
            $strprecio=number_format($row->precio,$decimalesprecio);
        ...
        if (($tipodoc == 2) || ($habilitanotaventaiva == 1)){
          ...
          $tabla.= "\r\n";
          $tabla.= "\r\n";
          $tabla.= "    SUBTOTAL  IVA12:" . chr(32) . chr(32) . number_format($resfact->subtotaliva,2) . "\r\n";
          $tabla.= "    SUBTOTAL   IVA0:" . chr(32) . chr(32) . number_format($resfact->subtotalcero,2) . "\r\n";
          $tabla.= "    DESCUENTO      :" . chr(32).chr(32). number_format($resfact->descmonto,2) . "\r\n";          
          $tabla.= "    MONTO IVA12    :" . chr(32) . chr(32) . number_format($resfact->montoiva,2) . "\r\n";
        }else{
          $totalpagar = $subtotaliva + $subtotalcero;
          $tabla.= "\r\n";
          $tabla.= "\r\n";
          $tabla.= "    SUBTOTAL       :" . chr(32) . chr(32) . number_format($resfact->subtotaliva + $resfact->subtotalcero,2) . "\r\n";
          $tabla.= "    DESCUENTO      :" . chr(32).chr(32). number_format($resfact->descmonto,2) . "\r\n";
        }
        ...
        $tabla.= "    TOTAL FACTURA  :" . chr(32) . chr(32) . number_format($resfact->monto,2) . "\r\n";
        ...
        $tabla.= "----------------------" . "\r\n";
        $tabla.= "La Factura será enviada a su correo electrónico." . "\r\n". "\r\n";
...
/*
    public function imprimirventa(){
        ...
        if($tipodoc == 2){
          ...
          // Subtotal c/Desc IVA (12%) 
          for ($i=0; $i <= 55; $i++) { 
            $tabla.= chr(32);
          } 
          $tabla.= "Subtotal c/Desc IVA (12%):";
...
*/
    ...
    public function reporteventaXLS(){
        ...
        $this->excel->getActiveSheet()->getColumnDimension('S')->setWidth(10);//aumento
        ...
        $this->excel->getActiveSheet()->setCellValue('S3', 'Base Imponible 12');//aumento
        $this->excel->getActiveSheet()->setCellValue('T3', 'Base Imponible 0');
        $this->excel->getActiveSheet()->setCellValue('U3', 'IVA 12');
        $this->excel->getActiveSheet()->setCellValue('V3', 'Subtotal');
        $this->excel->getActiveSheet()->setCellValue('W3', 'Monto Total');
        $this->excel->getActiveSheet()->setCellValue('X3', 'Ret IVA');
        $this->excel->getActiveSheet()->setCellValue('Y3', 'Ret Renta');
        ...
        $this->excel->getActiveSheet()->getStyle('R3')->getFont()->setBold(true);//AUMENTO
        ...
        foreach ($venta as $ven) {
          if($ven->estatus != 3){
            ...
            $bimponible12 += $ven->subconiva;//aumento
            $bimponible0 += $ven->subsiniva;
            $iva12 += $ven->montoiva;
            $subtotal += $ven->subtotal;
            $montototal += $ven->montototal;
            $retiva += $ven->retiva;
            $retrenta += $ven->retrenta;
            ...
            $this->excel->getActiveSheet()->setCellValue('S'.$fila, number_format($ven->subconiva,2));//aumento
            $this->excel->getActiveSheet()->setCellValue('T'.$fila, number_format($ven->subsiniva,2));
            $this->excel->getActiveSheet()->setCellValue('U'.$fila, number_format($ven->montoiva,2));
            $this->excel->getActiveSheet()->setCellValue('V'.$fila, number_format($ven->subtotal,2));
            $this->excel->getActiveSheet()->setCellValue('W'.$fila, number_format($ven->montototal,2));
            $this->excel->getActiveSheet()->setCellValue('X'.$fila, number_format($ven->retiva,2));
            $this->excel->getActiveSheet()->setCellValue('Y'.$fila, number_format($ven->retrenta,2));
        ...
        $this->excel->getActiveSheet()->setCellValue('S'.$fila, number_format($bimponible12,2));//aumento
        $this->excel->getActiveSheet()->setCellValue('T'.$fila, number_format($bimponible0,2));
        $this->excel->getActiveSheet()->setCellValue('U'.$fila, number_format($iva12,2));
        $this->excel->getActiveSheet()->setCellValue('V'.$fila, number_format($subtotal,2));
        $this->excel->getActiveSheet()->setCellValue('W'.$fila, number_format($montototal,2));
        $this->excel->getActiveSheet()->setCellValue('X'.$fila, number_format($retiva,2));
        $this->excel->getActiveSheet()->setCellValue('Y'.$fila, number_format($retrenta,2));
        ...
        $this->excel->getActiveSheet()->getStyle('S'.$fila)->getFont()->setBold(true);//AUMENTO
        $this->excel->getActiveSheet()->getStyle('T'.$fila)->getFont()->setBold(true);
        $this->excel->getActiveSheet()->getStyle('U'.$fila)->getFont()->setBold(true);
        $this->excel->getActiveSheet()->getStyle('V'.$fila)->getFont()->setBold(true);
        $this->excel->getActiveSheet()->getStyle('W'.$fila)->getFont()->setBold(true);
    ...
    //busca direccion
    public function busca_direccion(){
      $dir = $this->input->post('dir');
      $resu = $this->Pedido_model->busca_direccion($dir);
      if(count($resu) > 0){ $mens = $resu[0];  }
      else { $mens = $resu[0]; }
      $arr['mens'] = $mens;
    ...
    public function valcliente(){
        ...
        /*Edicion multiempresa*/ 
        /*$idusu = $this->session->userdata("sess_id");
        $id_empresa = $this->Correo_model->getemp_usuario($idusu);
        $resu = $this->Pedido_model->valida_cliente($idcliente, $id_empresa->id_empresa);*/
        /*original
        */
    ...
    /* FUNCIONES ESPECIALES DE BUSQUEDA DE DIRECCION*/
    public function valclientedireccion(){
      $tmpArray=array();
      $dircli = $this->input->get('direccion');
      $data = $this->Pedido_model->valida_direccion($dircli);
      foreach ($data as $row) {
          $tmpArray[] = $row->direccion;
      }
      print json_encode($tmpArray);
  }
    ...
    public function listadoDataVent() {
        ...
        foreach ($registro as $row) {
          ...
          if($row->tarjetac > 0){
            $detalle_tarjeta = '<a href=\"#\" title=\"Ver Tarjeta\" id=\"'.$row->id_venta.'\" class=\"detalle_tarjeta\"><i class=\"fa fa-credit-card\" aria-hidden=\"true\"></i></a>';
          }
          else{
            $detalle_tarjeta = '';
          }
          ...
          if($row->tarjetad > 0){
            $detalle_tarjetad = '<a href=\"#\" title=\"Ver Tarjeta\" id=\"'.$row->id_venta.'\" class=\"detalle_tarjeta\"><i class=\"fa fa-credit-card\" aria-hidden=\"true\"></i></a>';
          }
          else{
            $detalle_tarjetad = '';
          }
          ...
          if($row->cheque > 0){
            $detalle_cheque = '<a href=\"#\" title=\"Ver Cheque\" id=\"'.$row->id_venta.'\" class=\"detalle_tarjetac\"><i class=\"fa fa-credit-card\" aria-hidden=\"true\"></i></a>';
          }
          else{
            $detalle_cheque = '';
          }
...
*/          
            ...
            $tabla.='{"estatus":"' . $row->estatus . '",
                      ...
                      "cheque":"'.$row->cheque.$detalle_cheque.'",
                      "tarjetac":"'.$row->tarjetac.$detalle_tarjeta.'",
                      "tarjetad":"'.$row->tarjetad.$detalle_tarjetad.'",
...
/* ===== FACTURACION DEPOSITO =============================================================================================================================*/
    public function factura_deposito(){
      ...
      $facturapdf = $this->Parametros_model->sel_facturapdf();
      $vendedor = $this->facturar_model->lst_vendedores($idusu);
...
/*      $data["nronv"] = $nronv;        */
      ...
      //obtener correo por defecto
      //$idusu = $this->session->userdata("sess_id");
      $id_empresa = $this->Correo_model->getemp_usuario($idusu);
      $corr = $this->Correo_model->sel_correo_empresa($id_empresa->id_empresa);
      //impresion de correo por defecto
      $correo_default = $corr->usuario;
      $data["correo_default"] = $correo_default;
...
/*      $descuento = $piefact->desc_monto;*/
    ...
    //  $this->pdf_f->SetY(-0.1);
      ...
      if(($tipodoc == 2) || ($habilitanotaventaiva == 1)){
        ...
        $this->pdf_f->Cell(170,-4,utf8_decode("Subtotal IVA (12 %)"),0,0,'R');
        ...
        $tmpy +=5;
        $this->pdf_f->SetXY(1,$tmpy);
        $this->pdf_f->Cell(170,-4,utf8_decode("Subtotal con Descuento IVA (12 %)"),0,0,'R');
        $this->pdf_f->Cell(25,-4,utf8_decode('$'.$subtotaldiva),0,1,'R');

        $tmpy +=5;
        $this->pdf_f->SetXY(1,$tmpy);
        $this->pdf_f->Cell(170,-4,utf8_decode("IVA (12%)"),0,0,'R');
    ...
    //  $this->pdf_f->SetY(-0.1);
      ...
      if($tipodoc == 2){
        ...
        $this->pdf_f->Cell(160,-4,utf8_decode("IVA (12%)"),0,0,'R');
        $this->pdf_f->Cell(25,-4,utf8_decode('$'.$montoiva),0,1,'R');

        $this->pdf_f->Cell(160,-4,utf8_decode("Subtotal con Descuento IVA (0 %)"),0,0,'R');
        $this->pdf_f->Cell(25,-4,utf8_decode('$'.$subtotaldcero),0,1,'R');

        $this->pdf_f->Cell(160,-4,utf8_decode("Subtotal con Descuento IVA (12 %)"),0,0,'R');
        $this->pdf_f->Cell(25,-4,utf8_decode('$'.$subtotaldiva),0,1,'R');
        ...
        $this->pdf_f->Cell(160,-4,utf8_decode("Subtotal IVA (12 %)"),0,0,'R');
        $this->pdf_f->Cell(25,-4,utf8_decode('$ '.$subtotaliva),0,1,'R');
        */
      }else{
        $this->pdf_f->Ln(10);
...
// 
    ...
    /* public function listadoDataArea() {
      $registro = $this->facturar_model->sel_bestproductos();
      $tabla = "";
      foreach ($registro as $row) {
    ...
    public function sel_bestproductos() {
      date_default_timezone_set("America/Guayaquil");
      $registro = $this->facturar_model->sel_bestproductos(); /* bg-orange-active color-palette */

      $tabla = "";
      foreach ($registro as $row) {
        $tabla.='{"ventas":"' . $row->cantidadtotal . '",
                  "valor":"' . $row->total . '",
                  "producto":"'.$row->pro_nombre.'"},';
      }
      $tabla = substr($tabla, 0, strlen($tabla) - 1);
