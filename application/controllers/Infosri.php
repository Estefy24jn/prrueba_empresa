...
class InfoSRI extends CI_Controller {
    ...
    public function index(){
      ...
      //Obtener la fecha de caducidad de firma 
      $idusu = $this->session->userdata("sess_id");
      $empresa = $this->Correo_model->getemp_usuario($idusu);
      $datosemp = $this->Empresa_model->sel_emp_id($empresa->id_empresa);
      $config = array(
        'file' => FCPATH.'public/archivos/token/'.$datosemp->tokenfirma,
        'pass' => $datosemp->contrasena
      );
      $firma = new \Shara\Comprobantes\Firma($config, '');
      $resp =$firma->verificarCertPKey();
      $fechavalido = $firma->getValidoHasta();
      //$firma->tokenfirma, $firma->contrasena
      $data["firma"] = $fechavalido;
      $data["desde"] = $desde;
      $data["hasta"] = $hasta;
...
/*        $config = array(
       ...
       // var_dump($xml);
        ...
        //var_dump(file_exists($signedfile));
...
//      $id = 64;
      ...
      if ($this->compSRI != null){
        $this->compSRI->id = $id;
        $erroresLog .= "Comprobante - Id: " . $this->compSRI->id ." \n";
      }
      else {
        return false;
      }
      ...
      if (trim($comprobante) != ''){
        ...
        $erroresLog .= "Comprobante: " . $comprobante ." \n";
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
          ...
          //$erroresLog .= "Firma1: " . $archivofirmado ." \n";
        }
        else{
          //$erroresLog .= "EnviandoFirma: " . $comprobante . "Token: ". $objemp->tokenfirma . "contra:". $objemp->contrasena. " \n";
          $archivofirmado = $this->Firmar($comprobante, $objemp->tokenfirma, $objemp->contrasena);
          //$erroresLog .= "Firma2: " . implode($archivofirmado) ." \n";
        }
        ...
        if ($archivofirmado == true) {        
          $erroresLog .= "Archivo Firmado: " . $archivofirmado ." \n";
          ...
          //var_dump($res);
          $erroresLog .= "Estado: " . $res["estado"] ." \n";
          if ($res["estado"] == "RECIBIDA"){
            $erroresLog .= "Archivo Recibido: " . $res["estado"] ." \n";
      ...
      $arr['resu'] = $autorizado;
      $erroresLog .= "Autorizado: " . $autorizado ." \n";
      if ($autorizado != 1){
        //Comprueba si el mensaje de error es un array
        if(is_array($this->mensajerror)){
          
          //$erroresLog .= "Array: " . "es un array" ." \n";
          //$arr['mensaje'] = implode($this->mensajerror ) . $erroresLog;  
          //Comprobar que error es secuencial registrado 
          //$erroresLog .= $this->mensajerror[0] ."logerr1 \n";
          //Si el error es el 45 o 43 arreglar automaticamente
          if($this->mensajerror[0]=="(ERROR-45)ERROR SECUENCIAL REGISTRADO" || $this->mensajerror[0]=="(ERROR-43)CLAVE ACCESO REGISTRADA"){
            $this->compSRI->actualiza_enviado($comprobante);
            $res = $this->autorizarWs($comprobante, $strtmpambiente);
            $erroresLog .= $res ." Reparación automática \n";
            $autorizado = 1;
              //var_dump($res);
              $fechaautorizo = $this->crearRide($comprobante);  
              $fechaautorizo = str_replace('/','-', $fechaautorizo); 
          ...
          $erroresLog .= "No es Array: " . "No es un array" ." \n";
          $arr['mensaje'] = $this->mensajerror . $erroresLog;
        }
...
*/
    ...
    public function autorizarWs($claveAcceso, $tipoAmbiente=1){
      ...
      //$client = new soapClient($url);
      $client = new soapClient($url, array("connection_timeout" => 180));
    ...
    function crearRide($claveAcceso){
      ...
      $pdf->SetAuthor('Gestión Electrónica Ecuador');
...
//      $pdf->Output('My-File-Name.pdf', 'I');
    ...
    public function enviarCorreoClienteSRI($objfac, $claveAcceso) {
      //$correo = $this->Correo_model->env_correo($objfac->id_empresa); //metodo anterior
      $idusu = $this->session->userdata("sess_id");
      $empresa = $this->Correo_model->getemp_usuario($idusu);
      $correo = $this->Correo_model->sel_correo_empresa($empresa->id_empresa);
      ...
      $this->email->message($this->compSRI->obtenerMensajeCorreo($objfac, $claveAcceso));
      $this->email->to($mailcli /*'cristianfcalle@gmail.com'*/);
