    ...
    public function generar_ats_xml() {
        ...
        foreach ($registro as $row) {
          if ($facant != $row->id_comp){
            ...
            /* //Se necesita un array aqui, para tomar todas las retenciones que se hagan
            $retenciones = $this->Reporte_model->lst_ats_variasretenciones($empresa, $row->secuencialfac, $desde, $hasta);
             *///Para comprobar si tiene retenciones
          ...
          if(!empty($retenciones) and $row->codretrenta){
            foreach ($retenciones as $reten) {
              $detalleret = $air->addChild('detalleAir');
              $detalleret->addChild('codRetAir', $reten->codretrenta);
              $detalleret->addChild('baseImpAir', $reten->baseretrenta);
              $detalleret->addChild('porcentajeAir', $reten->porciento_retencion_renta);
              $detalleret->addChild('valRetAir', $reten->valor_retencion_renta);
            }
          }
          //ANTERIOR
          /* if ($row->codretrenta) {
        ...
        if (count($registro) > 0){
          ...
          foreach ($registro as $row) {
              ...
              //Para comprobar si tiene formas de pago o si no tiene mejor no poner esta casilla
              if(!empty($pagos)){
                $formapago = $detalle->addChild('formasDePago');
...
/*     $desde = "2017-10-01";
    ...
    public function reportesri01(){
      date_default_timezone_set("America/Guayaquil");
      $desde = $this->session->userdata("tmp_sri_desde");
      $hasta = $this->session->userdata("tmp_sri_hasta");
      $idusu = $this->session->userdata("sess_id");
      $utilidad = $this->InfoSRI_model->lst_venta_rango($desde, $hasta, $idusu); 

      $this->excel->setActiveSheetIndex(0);
      $this->excel->getActiveSheet()->setTitle('Reporte SRI');
      $this->excel->getActiveSheet()->setCellValue('A1', 'Reporte SRI');
      $this->excel->getActiveSheet()->getStyle('A1')->getFont()->setSize(16);
      $this->excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true);
      $this->excel->getActiveSheet()->mergeCells('A1:D1');
      $this->excel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
      ...
      $this->excel->getActiveSheet()->setCellValue('A3','Fecha');   
      $this->excel->getActiveSheet()->setCellValue('B3','#Documento');   
      $this->excel->getActiveSheet()->setCellValue('C3','C.I./R.U.C');   
      $this->excel->getActiveSheet()->setCellValue('D3','Cliente');   
      $this->excel->getActiveSheet()->setCellValue('E3','Base IVA 12');
      $this->excel->getActiveSheet()->setCellValue('F3','Base IVA 0%');   
      $this->excel->getActiveSheet()->setCellValue('G3','Monto IVA');   
      $this->excel->getActiveSheet()->setCellValue('H3','Descuento');   
      $this->excel->getActiveSheet()->setCellValue('I3','Monto Total');   
      ...
      $fila = 4;
      foreach ($utilidad as $utl) {
        $costototal = $costototal + $utl->montototal;
        // $montoiva = $montoiva + $utl->montoiva;
        // $desc = $desc + $utl->descuento;

        $fec = str_replace('-', '/', $utl->fecha); $fec = date("d/m/Y", strtotime($fec));  
        $this->excel->getActiveSheet()->setCellValue('A'.$fila, $fec);
        $this->excel->getActiveSheet()->setCellValue('B'.$fila, $utl->nro_factura);
        $this->excel->getActiveSheet()->setCellValue('C'.$fila, $utl->nro_ident);
        $this->excel->getActiveSheet()->getStyle('C'.$fila)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
        $this->excel->getActiveSheet()->setCellValue('D'.$fila, $utl->nom_cliente);
        $this->excel->getActiveSheet()->setCellValue('E'.$fila, $utl->descsubconiva);
        $this->excel->getActiveSheet()->setCellValue('F'.$fila, $utl->descsubsiniva);
        $this->excel->getActiveSheet()->setCellValue('G'.$fila, $utl->montoiva);
        $this->excel->getActiveSheet()->setCellValue('H'.$fila, $utl->desc_monto);
        $this->excel->getActiveSheet()->setCellValue('I'.$fila, $utl->montototal);
      ...
      $this->excel->getActiveSheet()->setCellValue('H'.$fila, 'TOTAL');
      $this->excel->getActiveSheet()->setCellValue('I'.$fila, number_format($costototal,2));
      $this->excel->getActiveSheet()->getStyle('H'.$fila.':'.'I'.$fila)->getFont()->setBold(true);
        foreach(range('A','I') as $columnID)
        {
            $this->excel->getActiveSheet()->getColumnDimension($columnID)->setAutoSize(true);
        }
        $this->excel->getActiveSheet()->freezePane('A4');
      $filename='reportesri01.xlsx'; //save our workbook as this file name
      header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); //mime type
      header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
      header('Cache-Control: max-age=0'); //no cache
    ...
    public function reportesri02(){
      date_default_timezone_set("America/Guayaquil");
      $desde = $this->session->userdata("tmp_sri_desde");
      $hasta = $this->session->userdata("tmp_sri_hasta");
      $idusu = $this->session->userdata("sess_id");
      $utilidad = $this->InfoSRI_model->lst_notacredito_rango($desde, $hasta, $idusu); 
      $this->excel->setActiveSheetIndex(0);
      $this->excel->getActiveSheet()->setTitle('Reporte SRI');
      $this->excel->getActiveSheet()->setCellValue('A1', 'Reporte SRI');
      $this->excel->getActiveSheet()->getStyle('A1')->getFont()->setSize(16);
      ...
      $this->excel->getActiveSheet()->getStyle('I'.$fila.':'.'R'.$fila)->getFont()->setBold(true);
        ...
        $this->excel->getActiveSheet()->freezePane('A4');
      $filename='reportesri02.xlsx'; //save our workbook as this file name
      header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); //mime type
      header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
      header('Cache-Control: max-age=0'); //no cache
