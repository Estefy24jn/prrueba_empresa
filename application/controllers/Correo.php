...
class Correo extends CI_Controller {
    ...
    public function index() {
        //$correo = $this->Correo_model->sel_correo(); //metodo anterior
        //Seleccionar el correo propio de cada empresa que esta el usuario
    ...
    public function guardar() { 
        ...
        //Para tomar la sesion de usuario y validar la empresa
        $idusu = $this->session->userdata("sess_id");
        $id_empresa = $this->Correo_model->getemp_usuario($idusu);
        if($id_empresa){
            //print $id_empresa->id_empresa;
            $add = $this->Correo_model->add_correo_empresa($smtp, $puerto, $user, $pwd, $id_empresa->id_empresa);
            ?>
            <script type="text/javascript"> alert("LOS DATOS FUERON ACTUALIZADOS ")</script>
        <?php
        }else{
            print "Usuario no pertenece a ninguna empresa";
        ?>
            <script type="text/javascript"> alert("USUARIO NO PERTENECE A NINGUNA EMPRESA ")</script>
        <?php  
        }
...
*/
    ...
    public function correoenviar() {
      ...
      $config = array(
        ...
        $this->email->to($correo->usuario);
         //$this->email->to('cristianfcalle@gmail.com.com');
         if($this->email->send(FALSE)){
            return 1;
    ...
    public function sendEmail($email,$subject,$message) { 
        $config = Array( 'protocol' => 'smtp', 
                         ...
                         'smtp_port' => 465, 
                         'smtp_user' => 'cristianfcalle@gmail.com', 
                         'smtp_pass' => 'ariel0956999721', 
                         'mailtype' => 'html', 
                         'charset' => 'iso-8859-1', 
                         'wordwrap' => TRUE ); 
        $this->load->library('email', $config); 
