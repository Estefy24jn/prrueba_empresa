...
class Puntoemision_model extends CI_Model {
    ...
    public function sel_puntoemision(){
      ...
      $idusu = $usua->id_usu;
      $perfil = $usua->perfil;
      $consulta = "SELECT DISTINCT p.id_puntoemision, p.id_sucursal, p.cod_establecimiento, 
      ...
      INNER JOIN permiso_sucursal ps on ps.id_sucursal = s.id_sucursal "; /* consumidor final */
        
    if ($perfil != 1) { //Cualquier usuario no administrador
        //Buscamos solo los puntos de emision que tiene permiso el usuario
        $consulta = $consulta . " WHERE ps.id_usuario = $idusu ";
    }else{
        //No hago nada si es administrador, listo todos los datos
    }
    $consulta = $consulta . " ORDER BY p.cod_establecimiento, p.cod_puntoemision ";

     /*  $query = $this->db->query("SELECT DISTINCT p.id_puntoemision, p.id_sucursal, p.cod_establecimiento, 
                                        p.cod_puntoemision, p.consecutivo_factura, p.consecutivo_notaventa, 
                                        p.consecutivo_comprobpago, p.activo, s.nom_sucursal, 
                                        p.consecutivo_notacredito, p.enviosriguardar_factura,
                                        p.consecutivo_retencioncompra, p.consecutivo_guiaremision,
