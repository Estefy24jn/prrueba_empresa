...
class Retencion_model extends CI_Model {
    ...
    public function retencioncompra_defaultadd($idcompra){
        ...
        if ($result[0]->cont == 0){
          $query = $this->db->query("select d.id_comp, p.id_cto_retencion, c.fecha, c.montoice, c.id_sucursal,
                                           ...
                                           0 as porciento_retencion_iva, 
                                           0 as valor_retencion_iva, 
                                           7 as id_porcentaje_retencion_iva
                                      from producto p
          ...
          foreach ($result as $ret) {
            ...
            if ($cant == 0){
              ...
              $query = $this->db->query("SELECT id_puntoemision from punto_emision where id_sucursal=$ret->id_sucursal order by cod_puntoemision LIMIT 1");
              $resret = $query->result();
              if ($resret)
    ...
    public function retencioncompra_guardar($idcompra, $autorizacion, $fecha, $iva10, $iva20, $iva30, $iva50, $iva70, $iva100, 
                                            $ptoemision, $nroretencion){
      //Conocer si existe una retencion para esa compra.
      $query = $this->db->query("SELECT id_comp_ret
        from compra_retencion where id_compra=$idcompra");
      $queryret = $query->result();
      if ($queryret) {
        //Si existe actualizar la retencion
        $id_retencion = $queryret[0]->id_comp_ret;
        $this->db->query("UPDATE compra_retencion set
          nro_autorizacion = '$autorizacion', 
          fecha_retencion = '$fecha'
          WHERE id_comp_ret = $id_retencion");

        //Hacer retencion en renta, eliminando la anterior
        $sql = $this->db->query("DELETE FROM compra_retencion_detrenta WHERE id_comp_ret = $id_retencion");
      
        $usua = $this->session->userdata('usua');
        $idusu = $usua->id_usu;
        $sql = $this->db->query("INSERT INTO compra_retencion_detrenta (id_comp_ret, id_concepto_retencion, base_noiva, base_iva, 
                                                                        porciento_retencion_renta, valor_retencion_renta)
                                  SELECT $id_retencion, id_concepto_retencion, base_noiva, base_iva, 
                                          porciento_retencion_renta, valor_retencion_renta
                                    FROM compra_retencion_detrenta_tmp
                                    WHERE id_usu = $idusu");

        $sql = $this->db->query("DELETE FROM compra_retencion_detrenta_tmp WHERE id_usu = $idusu");
        // Hacer retencion IVA eliminando la anterior
        $sql = $this->db->query("DELETE FROM compra_retencion_detiva WHERE id_comp_ret = $id_retencion");
        if ($iva10 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                     VALUES ($id_retencion, 1, $iva10 / 0.1, 10, $iva10)");
        }
        if ($iva20 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                     VALUES ($id_retencion, 2, $iva20 / 0.2, 20, $iva20)");
        }
        if ($iva30 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                     VALUES ($id_retencion, 3, $iva30 / 0.3, 30, $iva30)");
        }
        if ($iva50 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                     VALUES ($id_retencion, 4, $iva50 / 0.5, 50, $iva50)");
        }
        if ($iva70 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                     VALUES ($id_retencion, 5, $iva70 / 0.7, 70, $iva70)");
        }
        if ($iva100 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                     VALUES ($id_retencion, 6, $iva100, 100, $iva100)");
        }

      
      }else{
          //Si no existe crear una retencion nueva
          //Crear retencion nueva
          //Obtener datos del punto de venta para retencion                                         
          $query = $this->db->query("SELECT concat(cod_establecimiento,'-',cod_puntoemision,'-',LPAD(consecutivo_retencioncompra, 9, '0')) as numero
          from punto_emision where id_puntoemision=$ptoemision");
        $resret = $query->result();
        if ($resret) {$nroret = $resret[0]->numero;}
        //Insertar la retencion
        $query = $this->db->query("INSERT INTO compra_retencion (id_compra, nro_retencion, nro_autorizacion, fecha_retencion, id_puntoemision)
                                                  VALUES($idcompra, '$nroret', '$autorizacion', '$fecha', $ptoemision);");
        //Obtener el id de la ultima retencion
        $query = $this->db->query("SELECT max(id_comp_ret) as maxid FROM compra_retencion");
        $resret = $query->result();
        $id_comp_ret = $resret[0]->maxid;
        //Hacer retencion en renta
        $usua = $this->session->userdata('usua');
        $idusu = $usua->id_usu;
        $sql = $this->db->query("INSERT INTO compra_retencion_detrenta (id_comp_ret, id_concepto_retencion, base_noiva, base_iva, 
                                                                        porciento_retencion_renta, valor_retencion_renta)
                                  SELECT $id_comp_ret, id_concepto_retencion, base_noiva, base_iva, 
                                          porciento_retencion_renta, valor_retencion_renta
                                    FROM compra_retencion_detrenta_tmp
                                    WHERE id_usu = $idusu");
        ...
        if ($iva10 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                    VALUES ($id_comp_ret, 1, $iva10 / 0.1, 10, $iva10)");
        }
        if ($iva20 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                    VALUES ($id_comp_ret, 2, $iva20 / 0.2, 20, $iva20)");
        }
        if ($iva30 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                    VALUES ($id_comp_ret, 3, $iva30 / 0.3, 30, $iva30)");
        }
        if ($iva50 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                    VALUES ($id_comp_ret, 4, $iva50 / 0.5, 50, $iva50)");
        }
        if ($iva70 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                    VALUES ($id_comp_ret, 5, $iva70 / 0.7, 70, $iva70)");
        }
        if ($iva100 > 0){
          $sql = $this->db->query("INSERT INTO compra_retencion_detiva (id_comp_ret, id_porcentaje_retencion_iva, base_retencion_iva, porciento_retencion_iva, valor_retencion_iva)
                                    VALUES ($id_comp_ret, 6, $iva100, 100, $iva100)");
        }
    ...
    //ESTO ERA ANTERIOR PARA UNA AUTOMATICA
