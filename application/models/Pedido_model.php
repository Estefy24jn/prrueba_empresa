...
class Pedido_model extends CI_Model {
    ...
    //public function valida_cliente($idcliente, $id_empresa){
...
//      }
    ...
    /* BUSQUEDA POR DIRECCION - EN PROCESO */
    public function valida_direccion($dircli){
      /*Para multiempresa*/
      $idusu = $this->session->userdata("sess_id");
      $id_empresa = $this->Correo_model->getemp_usuario($idusu);
      $query = $this->db->query("SELECT direccion_cliente as direccion FROM clientes WHERE direccion_cliente like '%$dircli%' and id_empresa = $id_empresa->id_empresa");//borrado and id_empresa = $id_empresa->id_empresa
      /*original
      $query = $this->db->query("SELECT direccion_cliente as direccion FROM clientes WHERE nom_cliente like '%$nomcli%'");
      */
      $result = $query->result();
    ...
    public function busca_direccion($dir){
      /* $query = $this->db->query("SELECT id_cliente, nom_cliente, tipo_ident_cliente, ident_cliente,  
                                        nivel_est_cliente, ref_cliente, codigo,
                                        correo_cliente, ciudad_cliente, relacionado, direccion_cliente,  
                                        telefonos_cliente, mayorista, tipo_precio,
                                        codigo, credito, placa_matricula, id_vendedor
                                FROM  clientes
                                WHERE direccion_cliente = '$nom' LIMIT 1"); */
        $query = $this->db->query("SELECT direccion_cliente 
                                FROM  clientes
                                WHERE direccion_cliente = '$dir' LIMIT 1");
      $result = $query->result();
      if($result == NULL){
        $result = NULL;
        return $result;
