...
class Contab_update_model extends Update_base_model {
    ...
    public function actualizabase(){
        ...
        $this->actualiza_procalm_con_perdidasyganancias();
    ...
    $query = $this->db->query("
                    ...
                    SELECT c.id, c.codigocuenta, 
                           IFNULL((SELECT SUM(d.valor * (case d.debitocredito when 1 then 1 else -1 end))
                                     FROM con_comprobantedetalle d 
                                        INNER JOIN con_comprobante a on a.id = d.idcomprobante                                         
                                     WHERE d.idcuenta = c.id and
                                           (vsucursalid = 0 or a.idsucursal = vsucursalid) and c.idgrupocuenta IN (1,2,3) and
        ...
        /* PARA SOLO SACAR CUENTAS DE PYG */
        /* and c.idgrupocuenta IN (4,5) CON ESTO */
        public function actualiza_procalm_con_perdidasyganancias(){
            $query = $this->db->query("DROP PROCEDURE IF EXISTS `con_balance_pyg`;");
            $query = $this->db->query("
                CREATE PROCEDURE `con_balance_pyg`(
                    vsucursalid int,
                    vnivel int,
                    vdateto date,
                    vpendiente int
                ...
                BEGIN
                        ...
                        SELECT c.id, c.codigocuenta, 
                               IFNULL((SELECT SUM(d.valor * (case d.debitocredito when 1 then 1 else -1 end))
                                         FROM con_comprobantedetalle d 
                                            INNER JOIN con_comprobante a on a.id = d.idcomprobante                                         
                                         WHERE d.idcuenta = c.id and
                                               (vsucursalid = 0 or a.idsucursal = vsucursalid) and c.idgrupocuenta IN (4,5) and
                                               a.fechaasiento <= vdateto and
                                               ((vpendiente = 1 AND a.idestado <= 2)  OR (a.idestado = 2))), 0) as valor                          
                          FROM con_plancuenta c
                          WHERE c.esmovimiento = 1;                          
                      ...
                      SELECT * FROM tblcuenta ORDER BY valor, codigocuenta;   
                END");
            }
