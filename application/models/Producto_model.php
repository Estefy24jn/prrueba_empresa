...
/*
    ...
    public function lista_pro(){
      ...
      */
        ...
        $id = $usua->id_usu;
        $perfil = $usua->perfil;
        $consulta = "SELECT pro_id, pro_codigobarra, pro_codigoauxiliar, 
        pro_nombre, pro_descripcion, pro_preciocompra, pro_precioventa,
      (select sum(existencia) from almapro a where a.id_pro=producto.pro_id) as pro_existencia FROM producto "; 
        if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
        $consulta = $consulta . " INNER JOIN almapro ap ON ap.id_pro = producto.pro_id
...
/*
  ...
  public function selexistencia($idpro){
    ...
    $perfil = $usua->perfil;
    $consulta = "SELECT a.almacen_id, a.almacen_nombre, IFNULL(al.existencia,0) as existencia
    FROM almacen a
    LEFT JOIN almapro al ON al.id_alm = a.almacen_id AND al.id_pro = $idpro "; /* consumidor final */
    if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
        $consulta = $consulta . " INNER JOIN permiso_almacen pa ON pa.id_almacen = a.almacen_id
                                WHERE pa.id_usuario = $id ";
    } 
