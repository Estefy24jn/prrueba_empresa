...
class Compra_model extends CI_Model {
    ...
    public function ini_temp($idusu, $eliminatmp = 0){
      ...
      if($valor == 0){
        $fecha = date("Y-m-d");
        $sql = $this->db->query("INSERT INTO tmp_compra (id_usu, fecha, id_sucursal, cod_sri_tipo_doc, cod_sri_sust_comprobante, id_almacen, categoria)
                                   SELECT $idusu, '$fecha', (SELECT id_sucursal FROM sucursal 
                                    ORDER BY nom_sucursal LIMIT 1), 01, 01,
                                    (SELECT almacen_id FROM almacen ORDER BY almacen_id LIMIT 1),
                                    (SELECT id_cat_gas FROM gastos_categorias ORDER BY id_cat_gas LIMIT 1);");
      }
    ...
    public function check_nro_factura_repetida($factura){
      $verifica = $this->db->query("SELECT COUNT(*) AS valor FROM compra WHERE nro_factura = '$factura'");
      $valver = $verifica->result();
      $valor = $valver[0]->valor;
      if($valor > 0){ return 1; }
      return 0;
    ...
    public function lst_producto($almacen = 0){
        /* Datos de almacenes mutiempresa ELIMINADO POR FALTA DE FUNCIONALIDAD
        */
      //$usua = $this->session->userdata('usua');
      //$id = $usua->id_usu;
      //$datos_empresa = $this->Correo_model->getemp_usuario($id);
      /* $perfil = $usua->perfil;
      $consulta = "SELECT pro.pro_id as id_pro, pro.pro_codigobarra, pro.pro_codigoauxiliar, 
     ...
     pro.pro_idunidadmedida as id_unimed, 
      ...
      if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
          ...
          $consulta = $consulta . " AND pro_aplicompra=1 AND pro_esservicio=0";
      }else{
          $consulta = $consulta . "WHERE pro_aplicompra=1 and pro_esservicio=0";
      }  */
...
/*      $sql = $this->db->query(" SELECT  id_comp, p.nom_proveedor,fecha, nro_factura, montototal, tc.nom_cancelacion, 
    ...
    public function guardar_compra($idtmp_comp, $fecha, $formapago, $efectivo, $tarjeta, $cambio, $dias, $cajachica, $categoria, $almacen){
      ...
      if($formapago == 'Contado'){ 
        ...
        $varid = $sqlid->result();
        $idpago = $varid[0]->id;
        /* AQui esta el error, pide tipo 2 y mi tipo es 1*/
        ...
        $this->db->query("INSERT INTO compra_abonos (iddocpago, id_compra, id_formapago, monto) 
                            VALUES ($idpago, $idcomp, 1, $monto)");/*$efectivo*/ /* $monto */
        $this->db->query("INSERT INTO compra_abonos (iddocpago, id_compra, id_formapago, monto) 
...
*/
    ...
    public function selcompret($idcomp){
      //Buscar descsubconiva Para monto ICE
      //(c.descsubconiva + ifnull(c.montoice,0)) as descsubconiva // Para monto ICE
      //PARA QUE NO SALGA LA RETENCION DEL 100%
      ...
      $sql = $this->db->query(" SELECT c.id_comp, r.id_comp_ret, r.id_puntoemision,
                                       ...
                                       (c.descsubconiva) as descsubconiva, c.fecha,
