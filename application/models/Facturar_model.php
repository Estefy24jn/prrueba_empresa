...
*/
  ...
  public  function data_cliente($nro_ident, $nom_cliente, $cor_cliente, $telf_cliente, $dir_cliente)
  {
    ...
    if ($valcli > 0) {
      ...
      if ($cor_cliente != NULL || $cor_cliente = "") {
      } else {
        $cor_cliente = " ";
      }
      if ($telf_cliente != NULL || $telf_cliente = "") {
      } else {
        $telf_cliente = " ";
      }
      if ($dir_cliente != NULL || $dir_cliente = "") {
      } else {
        $dir_cliente = " ";
      }
  ...
  public function venta_rango($desde, $hasta, $vendedor = 0, $sucursal = 0, $tipofecha = 1)
  {
    ...
    $sql = $this->db->query(" SELECT v.id_venta, c.nom_caja, v.fecha, v.nro_factura, tc.nom_cancelacion, v.mesa, 
                                       ...
                                       v.id_vendedor, v.observaciones, v.subconiva, v.subsiniva, v.montoiva,
                                ...
                                IFNULL((SELECT sum(monto) FROM venta_formapago p 
                                   ...
                                   ifnull((SELECT sum(dr.valor_retencion_renta) FROM venta_retencion_detrenta dr
                                              INNER JOIN venta_retencion r on r.id_venta_ret = dr.id_venta_ret
                                              WHERE r.id_venta = v.id_venta),0) as retrenta,
                                     ifnull((SELECT sum(dr.valor_retencion_iva) FROM venta_retencion_detiva dr
                                              INNER JOIN venta_retencion r on r.id_venta_ret = dr.id_venta_ret
                                              WHERE r.id_venta = v.id_venta),0) as retiva
                                FROM venta v 
                                ...
                                WHERE (DATE($strtipofecha) BETWEEN '$desde' AND '$hasta') AND
  ...
  public function anularycrear_factura(
    $id_venta,
    $causa,
    $fecha,
    $tipo_doc,
  ...
  public function carga_cliente($idusu, $nrofact)
  {
    ...
    if ($val == 0) {
      ...
      $tp_cli = "C"/*$clinvo[0]->tipo_ident_cliente*/;
      $id_cli = "9999999999"/*$clinvo[0]->ident_cliente*/;
      /*$id_cli = "9999999999999" Respaldo de RUC*/
  ...
  public function upd_ventcliente($idusu, $idcli, $idtp, $nom, $tel, $cor, $dir, $ciu, $idc, $placa)
  {
    ...
    if ($val > 0) {
      ...
      if ($valcli > 0) {//Si existe entonces actualizar
        ...
        if ($idc != 1) {
          /*Para ingrso de clientes multiempresa*/
          $idusu = $this->session->userdata("sess_id");
          $id_empresa = $this->Correo_model->getemp_usuario($idusu);
        ...
        return $tipo_doc;
      } else { //Si no existe entonces insertar cliente nuevo
        if ($cor != NULL || $cor = "") {
        } else {
          $cor = " ";
        }
 ...
 */
  ...
  public function pagar_facturageneral($idusu, $fp, $tipocancelacion)
  {
    ...
    if ($valcli > 0) {
      ...
      if ($ciu_cliente != NULL || $ciu_cliente = "") {
      } else {
        $ciu_cliente = " ";
      }
  ...
  {
    /* Datos de almacenes mutiempresa
      */
    $usua = $this->session->userdata('usua');
    $id = $usua->id_usu;
    $datos_empresa = $this->Correo_model->getemp_usuario($id);
    $perfil = $usua->perfil;
    $consulta = "SELECT almacen_id, almacen_nombre FROM almacen "; /* consumidor final */
    if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
      $consulta = $consulta . " WHERE sucursal_id = $datos_empresa->id_sucursal ";
    } 
    //------------------
  ...
  public function preciopro($idcliente = null)
  {
    ...
    if (count($resultado) > 0) {
      $categoriaventa = $resultado[0]->id_categoriaventa;
    }
  ...
  public function addforpago($idventa, $fp, $monto, $fechat, $tiptarjeta, $nrotar, $banco, $tbanco, $nrodoc, $tnrodoc, $descdoc, $tdescdoc, $fechae, $fechac, $nrocta, $fpvtc)
  {
    if ($fechat == NULL || $fechat == "") {
      ...
      $fecha = str_replace('/', '-', $fechat);
      $fechat = date("Y-m-d", strtotime($fecha));
    }
    if ($fechae == NULL || $fechae == "") {
    } else {
      $fecha = str_replace('/', '-', $fechae);
      $fechae = date("Y-m-d", strtotime($fecha));
    }
    if ($fechac == NULL || $fechac == "") {
    } else {
      $fecha = str_replace('/', '-', $fechac);
      $fechac = date("Y-m-d", strtotime($fecha));
    }
  ...
  public function updforpago($idreg, $idventa, $fp, $monto, $fechat, $tiptarjeta, $nrotar, $banco, $tbanco, $nrodoc, $tnrodoc, $descdoc, $tdescdoc, $fechae, $fechac, $nrocta)
  {
    ...
    $tarjeta = $tfp->tarjeta;
    $tipo = "";
    if ($tarjeta == 1 && $banco == 0) {
      $tipo = "Tarjeta";
    }
    if ($tarjeta == 0 && $banco == 1) {
      $tipo = "Banco";
    }
  ...
  public function selforpagovent($idventa)
  {
    /*
      $sql = $this->db->query(" SELECT ft.idreg, ft.id_formapago, f.nombre_formapago AS nomfp, ft.monto, ft.id_tipcancelacion
        ...
        */
    $sql = $this->db->query(" SELECT (CASE WHEN ft.id_formapago = 4 THEN IFNULL(t.comision_debito,0) ELSE IFNULL(t.comision_credito,0) END) as comision, ft.idreg, ft.id_formapago, f.nombre_formapago AS nomfp, ft.monto, ft.id_tipcancelacion
            FROM formapago_tmp ft
            INNER JOIN formapago f ON f.id_formapago = ft.id_formapago
  ...
  public function ventaformapagotarjetaporid($id_venta)
  {

    $sql = $this->db->query(" SELECT v.id_abono, v.id_banco, b.nombre, v.numerotarjeta, v.numerodocumento, 
                          v.fechaemision, v.descripciondocumento
                          FROM venta_formapagotarjeta v 
                          INNER JOIN venta_formapago vf ON vf.id = v.id_abono
                          INNER JOIN bancos b ON v.id_banco = b.id_banco
                          WHERE vf.id_venta = $id_venta");
    $resu = $sql->result();
    return $resu;
  }

  public function ventaformapagochequeporid($id_venta)
  {

    $sql = $this->db->query(" SELECT v.id_abono, v.id_banco, b.nombre, v.numerocuenta, v.numerodocumento, 
                          v.fechaemision, v.descripciondocumento
                          FROM venta_formapagobanco v 
                          INNER JOIN venta_formapago vf ON vf.id = v.id_abono
                          INNER JOIN bancos b ON v.id_banco = b.id_banco
                          WHERE vf.id_venta = $id_venta");
    $resu = $sql->result();
  ...
  public function lst_vendedores($idusu)
  {
    $sql = $this->db->query("SELECT id_usu, CONCAT(nom_usu,' ',ape_usu) AS vendedor FROM usu_sistemas WHERE id_usu = $idusu");
    $res = $sql->result();
    return $res;
  }
  ...
  //Funcion por Andres para Charts de Inicio
  public function lst_ventasanio()
  {
    $sql = $this->db->query("SELECT
    SUM(IF(MONTH(v.fecha) = 1,  v.montototal, 0)) AS Ene,
    SUM(IF(MONTH(v.fecha) = 2,  v.montototal, 0)) AS Feb,
    SUM(IF(MONTH(v.fecha) = 3,  v.montototal, 0)) AS Mar,
    SUM(IF(MONTH(v.fecha) = 4,  v.montototal, 0)) AS Abr,
    SUM(IF(MONTH(v.fecha) = 5,  v.montototal, 0)) AS May,
    SUM(IF(MONTH(v.fecha) = 6,  v.montototal, 0)) AS Jun,
    SUM(IF(MONTH(v.fecha) = 7,  v.montototal, 0)) AS Jul,
    SUM(IF(MONTH(v.fecha) = 8,  v.montototal, 0)) AS Ago,
    SUM(IF(MONTH(v.fecha) = 9,  v.montototal, 0)) AS Sep,
    SUM(IF(MONTH(v.fecha) = 10, v.montototal, 0)) AS Oct,
    SUM(IF(MONTH(v.fecha) = 11, v.montototal, 0)) AS Nov,
    SUM(IF(MONTH(v.fecha) = 12, v.montototal, 0)) AS Dic
    FROM venta v
    WHERE YEAR(v.fecha) = YEAR(NOW())");


    $resu = $sql->result();
    //$resu = [100, 100, 100, 80, 80, 80, 100, 100, 100, 100, 100, 55];
    return $resu;
  }

  public function lst_ventasmesactual(){
    $sql = $this->db->query("SELECT
    SUM(IF(DAY(v.fecha) = 1,  v.montototal, 0)) AS uno,
    SUM(IF(DAY(v.fecha) = 2,  v.montototal, 0)) AS dos,
    SUM(IF(DAY(v.fecha) = 3,  v.montototal, 0)) AS tres,
    SUM(IF(DAY(v.fecha) = 4,  v.montototal, 0)) AS cuatro,
    SUM(IF(DAY(v.fecha) = 5,  v.montototal, 0)) AS cinco,
    SUM(IF(DAY(v.fecha) = 6,  v.montototal, 0)) AS seis,
    SUM(IF(DAY(v.fecha) = 7,  v.montototal, 0)) AS siet,
    SUM(IF(DAY(v.fecha) = 8,  v.montototal, 0)) AS ocho,
    SUM(IF(DAY(v.fecha) = 9,  v.montototal, 0)) AS nue,
    SUM(IF(DAY(v.fecha) = 10, v.montototal, 0)) AS diez,
    SUM(IF(DAY(v.fecha) = 11, v.montototal, 0)) AS onca,
    SUM(IF(DAY(v.fecha) = 12, v.montototal, 0)) AS doca,
    SUM(IF(DAY(v.fecha) = 13, v.montototal, 0)) AS treca,
    SUM(IF(DAY(v.fecha) = 14, v.montototal, 0)) AS catoa,
    SUM(IF(DAY(v.fecha) = 15, v.montototal, 0)) AS quin,
    SUM(IF(DAY(v.fecha) = 16, v.montototal, 0)) AS diec,
    SUM(IF(DAY(v.fecha) = 17, v.montototal, 0)) AS dies,
    SUM(IF(DAY(v.fecha) = 18, v.montototal, 0)) AS dieo,
    SUM(IF(DAY(v.fecha) = 19, v.montototal, 0)) AS dien,
    SUM(IF(DAY(v.fecha) = 20, v.montototal, 0)) AS ve,
    SUM(IF(DAY(v.fecha) = 21, v.montototal, 0)) AS veu,
    SUM(IF(DAY(v.fecha) = 22, v.montototal, 0)) AS ved,
    SUM(IF(DAY(v.fecha) = 23, v.montototal, 0)) AS vet,
    SUM(IF(DAY(v.fecha) = 24, v.montototal, 0)) AS vecs,
    SUM(IF(DAY(v.fecha) = 25, v.montototal, 0)) AS vec,
    SUM(IF(DAY(v.fecha) = 26, v.montototal, 0)) AS ves,
    SUM(IF(DAY(v.fecha) = 27, v.montototal, 0)) AS vesi,
    SUM(IF(DAY(v.fecha) = 28, v.montototal, 0)) AS veoc,
    SUM(IF(DAY(v.fecha) = 29, v.montototal, 0)) AS venu,
    SUM(IF(DAY(v.fecha) = 30, v.montototal, 0)) AS tre,
    SUM(IF(DAY(v.fecha) = 31, v.montototal, 0)) AS trei
    FROM venta v
  ...
  public function sel_bestproductos(){

    $sql = $this->db->query("SELECT ROUND(SUM(d.cantidad),0) AS cantidadtotal, p.pro_nombre,
                                      p.pro_precioventa, CONCAT(ROUND(SUM(d.descsubtotal + d.montoiva),2),' $') AS total
                                  FROM venta_detalle d
                                  INNER JOIN venta v ON v.id_venta = d.id_venta
                                  INNER JOIN producto p on p.pro_id = d.id_producto
                                  WHERE MONTH(v.fecha) = MONTH(NOW())
  ...
  public function lst_formapago(){
    $sql = $this->db->query("SELECT
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 1 AND MONTH(v.fecha) = MONTH(NOW())),0)) as efectivo, 
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 2 AND MONTH(v.fecha) = MONTH(NOW())),0)) as cheque,
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 3 AND MONTH(v.fecha) = MONTH(NOW())),0)) as tarjetac, 
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 4 AND MONTH(v.fecha) = MONTH(NOW())),0)) as tarjetad, 
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 5 AND MONTH(v.fecha) = MONTH(NOW())),0)) as tarjetap,
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 6 AND MONTH(v.fecha) = MONTH(NOW())),0)) as transferencia,
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 7 AND MONTH(v.fecha) = MONTH(NOW())),0)) as dinele,
                                (ifnull((SELECT SUM(monto) FROM venta_formapago fp, venta v WHERE v.id_venta= fp.id_venta AND id_formapago = 8 AND MONTH(v.fecha) = MONTH(NOW())),0)) as otros");
