...
class Inventario_model extends CI_Model {
    ...
    /*FUNCION PARA INGRESAR KARDEX POR REST */
    public function ins_kardexegreso_rest($idpro, $documento, $detalle, $cantidad, $valunit, $costototal, $unidad,
                                     $id_almacen, $idusuario){
      $sqlid = $this->db->query("call kardexegreso_ins($idpro, '$documento', '$detalle', 
                                                        $cantidad, $valunit, $costototal, 
                                                        $unidad, $idusuario, $id_almacen);");
      $varobj = $sqlid->result();
    ...
    $perfil = $usua->perfil;
    $consulta = "SELECT almacen_id, almacen_nombre, almacen_direccion,
    almacen_responsable, almacen_descripcion, sucursal_id 
    FROM almacen "; 
    if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
        $consulta = $consulta . " INNER JOIN permiso_almacen pa ON pa.id_almacen = almacen.almacen_id
                                WHERE pa.id_usuario = $id ";
    } 
    $consulta = $consulta . " ORDER BY almacen_nombre ";
    ...
    public function lst_producto(){
      ...
      $sql = $this->db->query("SELECT pro.pro_id, pro.pro_codigobarra, pro.pro_codigoauxiliar, 
                                ...
                                INNER JOIN almapro ap ON ap.id_pro = pro.pro_id and ap.id_alm=ifnull((select id_almacen from tmp_movinv where id_usu=$idusu),0)
    ...
    public function lstproalma($idalm){
      //Seleccionar productos de todos los almacenes 0, uno especifico Se envía el ID  
      if($idalm == 0){
        $alm = " ";
      }else{
    ...
    //$datos_empresa = $this->Correo_model->getemp_usuario($id);
    $perfil = $usua->perfil;
    $consulta = "SELECT CONCAT(p.pro_id,'-',a.id_alm) AS pro_id, p.pro_codigobarra, p.pro_codigoauxiliar, 
    REPLACE(REPLACE(p.pro_nombre, '\r', ''), '\n', '') as pro_nombre,  
    ...
    if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
        $consulta = $consulta . " INNER JOIN permiso_almacen pa ON pa.id_almacen = al.almacen_id
                                WHERE pa.id_usuario = $id AND
                                p.preparado = 0 $alm";
    }else{//Para el caso de otros usuarios
        $consulta = $consulta ." WHERE p.preparado = 0 $alm";
    } 
    ...
    //$datos_empresa = $this->Correo_model->getemp_usuario($id);
    $perfil = $usua->perfil;
    $consulta = "SELECT pro.pro_id, pro.pro_codigobarra, pro.pro_codigoauxiliar, 
    REPLACE(REPLACE(pro.pro_nombre, '\r', ''), '\n', '') as pro_nombre,  
    ...
    FROM producto pro
    INNER JOIN unidadmedida um ON um.id = pro.pro_idunidadmedida 
    LEFT JOIN almapro ap ON ap.id_pro = pro.pro_id and ap.id_alm=$almacen "; /* consumidor final */
    if ($perfil != 1) { //Obtiene todos los almacenes si es administrador
        $consulta = $consulta . " INNER JOIN permiso_almacen pa ON pa.id_almacen = ap.id_alm
                                and pa.id_usuario = $id ";
    } 
    $consulta = $consulta ." WHERE ifnull(preparado,0)=0 ";
    $sql = $this->db->query($consulta);
